...
const RequestInfoDialog = props => {
  const { openRequestInfoDialog, closeCallback, requestRecord, classes } = props;
  ...
  const loadResultFile = requestRecord => {
    // pk = 1 -> mirinho
    if (requestRecord.tool.pk === 1) {
      ...
      const mirboostOutFileUrl = requestRecord.request.file.replace(
        requestRecord.request.fileName,
        `result_mirboost_${requestRecord.requestCode}.txt`
      );
      fetch(mirboostOutFileUrl)
        .catch(error => {
          toast.error('Ops! Something went wrong while trying to retrieve result file', {
    ...
    <div>
      <Dialog open={openRequestInfoDialog} TransitionComponent={Transition} fullScreen>
        ...
        <DialogContent className={classes.dialogContainer}>
          ...
          {value === 0 && (
            <Grid container className={classes.infoGrid} spacing={24}>
        ...
        <DialogActions>
          <Button variant="contained" onClick={ev => downloadFile(requestRecord, false)} color="primary">
            <SaveIcon className={classes.leftIcon} />
            Download Input File
          </Button>
          <Button variant="contained" onClick={ev => downloadFile(requestRecord, true)} color="primary">
            <SaveIcon className={classes.leftIcon} />
